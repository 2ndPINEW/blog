...
export function mdToHtmlAndJson(): void {
  ...
  fs.writeFileSync(
    getDistPagingJsonPath(0),
    JSON.stringify({ contents: metaDatas })
  );
  ...
  splitArray(metaDatas, 50).forEach((datas: MetaData[], index: number) => {
    fs.writeFileSync(
      getDistPagingJsonPath(index + 1),
      JSON.stringify({ contents: datas })
    );
  });
}
